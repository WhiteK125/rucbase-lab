...
class NestedLoopJoinExecutor : public AbstractExecutor {
   ...
   private:
    ...
    bool check_join_condition(RmRecord* left_rec, RmRecord* right_rec, const Condition& cond) {
        ...
        // 尝试在左表中查找左边的列
        auto lhs_col_it = std::find_if(left_cols.begin(), left_cols.end(), [&](const ColMeta& col) {
        ...
        if (lhs_col_it != left_cols.end()) {
            ...
            auto it = std::find_if(right_cols.begin(), right_cols.end(), [&](const ColMeta& col) {
                return col.tab_name == cond.lhs_col.tab_name && col.name == cond.lhs_col.col_name;
            });
        ...
        if (cond.is_rhs_val) {
            ...
            // 右边是列，查找它属于哪个表
            auto rhs_col_it = std::find_if(left_cols.begin(), left_cols.end(), [&](const ColMeta& col) {
