...
class SeqScanExecutor : public AbstractExecutor {
   ...
   private:
    ...
    bool compare_values(char *lhs, char *rhs, ColType type, int len, CompOp op) {
        ...
        // 根据比较操作符返回结果
        switch (op) {
            case OP_EQ: return cmp_result == 0;   // 等于
            case OP_NE: return cmp_result != 0;   // 不等于
            case OP_LT: return cmp_result < 0;    // 小于
            case OP_GT: return cmp_result > 0;    // 大于
            case OP_LE: return cmp_result <= 0;   // 小于等于
            case OP_GE: return cmp_result >= 0;   // 大于等于
            default:
